		...
		<div class="content-box-right">
            <ul>
                <% foreach (var x in ExtensionList())
                   {
                       var cls = "";
                       if (this.Request.RawUrl.IndexOf(x.Name, StringComparison.OrdinalIgnoreCase) != -1)
                           cls = "content-box-selected";

                       if (x.Settings.Count > 0 && x.Settings[0] != null && x.ShowSettings != false)
                           Response.Write(string.Format("<li class=\"{1}\"><a href=\"Settings.aspx?ext={0}\">{0}</a></li>", x.Name, cls));
                   }%>
            </ul>
		</div>
